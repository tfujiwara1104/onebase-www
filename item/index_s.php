...
<div class="div_floor_title">
	<img src="/img/common/floor_map_2th.png" alt="" />
	<h2 class="market_head_h2">Skill market</h2>
	<p class="market_head_p">2F スキル販売フロア</p>
</div>

<section class="section_research">
	<?=pagenavi_item($totalcnt2,$pno2)?>
	<p class="pagenavi_num"><?=$totalcnt2?>件中/<?=$startnum2+1?>件～<?=$endnum2?>件 を表示</p>
</section>

<div class="LINEUP_BOX">
	<?foreach($_item2 as $itid => $col){ extract($col);?>
    <?if($re_tax==2) $taxrate=$re_taxrate;?>
	<?if($disp==1 and $product_type==2){?>
	<div class="item_job">
		<a href="/item/<?=$itid?>">
			<div class="img_BOX02">
				...
				<?if(!$top_img){?><img style="object-fit: cover;height:100%;" src="/img/common/dummy.png" data-original="/img/<?=is_file("{$_SERVER['SERVER_ROOT']}/upfile/item/{$itid}/01")?"upfile/item/{$itid}/01":"common/noimage.png"?>" class="lazy" alt="" /><?}?></div>
				...
				<?if(isset($_item3[$itid]) and $sell_limit!=2 ){?>
				...
				<?if($enddate and $enddate < date("Y-m-d") and !(isset($_item3[$itid]) and $sell_limit==2 )){?>
